...
if __name__ == '__main__':
    ...
    # plugin for trajectory display
    visualpd = etree.SubElement(base_link, 'visual', name="visualpd")
    etree.SubElement(visualpd, 'plugin', name="draw_traj", filename="libdraw_traj.so")
    CreateNested(visualpd, 'geometry/sphere/radius', 0.5)
    CreateNested(visualpd, 'transparency', 1.0)
    ...
    if config.platform.type == 'sphere':
        pose = config.platform.position.xyz + config.platform.position.rpy
        CreateVisualCollision(link, 'pf/geometry/sphere/radius',config.platform.radius, collision=True, color=config.platform.color, mass = config.platform.mass, inertia=config.platform.inertia)
        # CreateNested(link, 'visualpf/plugin/name', 'draw_traj')
        # CreateNested(link, 'visualpf/plugin/filename', 'libdraw_traj.so')
        # plug = etree.SubElement(link, 'visual', name="visualpf")
        # plug = etree.SubElement(plug, 'plugin', name="draw_traj", filename="libdraw_traj.so")
    ...
    # <plugin filename="libdraw_traj.so" name="draw_traj"/>
    # 
    ...
    if sim_cables:
        ...
        for i, cbl in enumerate(config.points):
            ...
            CreateVisualCollision(link,'/geometry/cylinder/radius', config.cable.radius, color=config.cable.color, collision=False, mass = config.cable.mass)
            ...
            BuildInertial(link, config.cable.inertia[0])
            ...
            BuildInertial(link, config.cable.inertia[1])
            CreateNested(link, 'pose', '%f %f %f %f %f %f' % tuple(cbl.frame + rpy))
            ...
            CreateNested(joint, 'axis/limit/lower', -3*l)
            CreateNested(joint, 'axis/limit/upper', 3*l)    
            ...
            # CreateVisualCollision(link,'/geometry/cylinder/radius', .03, color='Yellow', collision=False)
            # CreateNested(link, 'visual/geometry/cylinder/length', 0.3)
            # revolute joint around X
            ...
            BuildInertial(link, config.cable.inertia[1])
            CreateNested(link, 'pose', '%f %f %f %f %f %f' % tuple(list(pp.reshape(3)) + rpy))
            # CreateVisualCollision(link,'/geometry/cylinder/radius', .03, color='Blue', collision=False)
            # CreateNested(link, 'visual/geometry/cylinder/length', 0.3)
            # revolute joint around Y
    ...
    # control plugin
    plug = etree.SubElement(model, 'plugin', name='cdpr_gazebo_plugin', filename='libcdpr_gazebo_plugin.so')
    '''
      <link name="ee">  
