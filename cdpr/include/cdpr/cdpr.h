...
protected:
    ...
    void PFState_cb(const gazebo_msgs::LinkStateConstPtr &_msg)
    {
        ...
        v_.resize(6);
        velocityPose[0] = v_[0]=_msg->twist.linear.x; 
        velocityPose[1] = v_[1]=_msg->twist.linear.y; 
        velocityPose[2] = v_[2]=_msg->twist.linear.z; 
        // std::cout<<velocityPose[2]<<std::endl;
    ...
    void Cables_cb(const cdpr::Tensions &_msg)
    {
        cables_ok = true;
        cable_states = _msg;
    }
